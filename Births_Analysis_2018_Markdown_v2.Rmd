...
#### iv
births$preterm_f <- cut(births$wksgest, breaks = c(0,36,99), labels =  c("preterm", "term")) #should be no births too young at this point
...
### Question 1, Summary Table for Crude Effects 
1.	Create a summary data.frame/table for crude effects: Using either dplyr:: functions or table() and prop.table() as we did in class, report the effect of early prenatal care on preterm birth by: 
	1.	the crude risk difference
	2. the crude risk ratio 
	3. the crude odds ratio 
...
### Question 2, Summary Table for EMM 
1.  Create a summary data.frame/table to report race-ethnicity specific crude RDs: again by using dplyr or (in this case) a three dimensional table()/prop.table(), report the risk difference effect of prenatal care on preterm birth by race-ethnicity status. Paste that table or text below.


<div class="fold o"> 
```{r, echo=T, warning=F, message=F, results='asis', fig.width=11, fig.height=8}
pt2 = prop.table(table(births$pnc5_f, births$preterm_f, births$raceeth_f),
                 margin = c(1,3)) #(HW4.1.2)

knitr::kable(pt2)


# ptb %>% 
#   dplyr::select(preterm, pnc5, mrace, raceeth_f) %>% 
#   dplyr::group_by(raceeth_f) %>% 
#   dplyr::summarise(CrudeRD = 
#                      prop.table(xtabs(~pnc5 + preterm), margin = 1)[2,2] - prop.table(xtabs(~pnc5 + preterm), margin = 1)[1,2])
...
### Question 3, Plot EMM 
1.  Create a graphic like that below showing the risk prevalence by exposure status and race-ethnicity.    
*Hint: the line (you could also use a ribbon) is created using geom_hline(). The text is from the annotate() function, which can be used to place any single geometry element directly on the graph without needing a data.frame or an aesthetic mapping.*


<div class="fold o"> 
```{r, echo=T, warning=F, message=F, results='asis', fig.width=11, fig.height=8}
#(HW4.1.3)
pt2_df = data.frame(pt2)
names(pt2_df) = c("exposure", "outcome", "group", "estimate")
...
## Homework 4, Part 2
### Question 4, Modeling 
1.  Using glm(), create a model object, m_crude_rd, that models the crude effect of prenatal care on preterm birth and report the risk difference (the non-intercept coefficient in the crude model when you print or ask a summary() of the model object. For risk differences, you’ll need the binomial family with the identity link. Confirm this risk difference matches your tabular result from part 1.

<div class="fold o"> 
```{r, echo=T, warning=F, message=F, results='asis', fig.width=11, fig.height=8}
#(HW4.2.4)
## just for demonstration purposes, have releveled the referent group for preterm (see next question below for difference)
...
### Question 6, Structure of the model  
1.	Describe the structure of the model object using str(). The model object can be large, so you may need the max.level parameter (set to 1) to reduce what’s printed. You do not have to describe every element, just the overall structure. Be able to select elements of the model structure using our usual syntax for the data structure type.
...
1.	Report the coefficients and confident intervals of this crude risk difference model using coef() and confint() functions.
...
1.	Use broom::tidy to turn the most commonly used model components into a data.frame. Use cbind() to combine broom::tidy()’s results with confidence intervals from conf.int() or broom:: confint_tidy()  – this is often a useful output structure for graphing.
...
### Question 9, Graphing Model Point Estimates  
1.	Use ggplot2 to plot your model results (point estimate and confidence interval) and paste in here. Perhaps a geom_point()+geom_errorbar()? If you don’t do the below challenge, you’ll just have one estimate and CI. Hint: I’m using (see below) two geom_point()s – white and colored - and a geom_linerange() with standard error color - mimics Tufte style boxplots.
...
### Question 10, Your Turn -- Graphing Model Point Estimates  
1.	Run additional models besides the crude, then use tools like data.frame(), subset operators (for example, to just pull out the PNC coefficient results) rbind() and cbind() to  build a single data.frame of all your results, and plot them using ggplot2. Here is an example, but yours could look different.   
  
  
**SEE ABOVE**

### Question 11, Risk Ratios and Odds Ratios
1.  **Optional**: Model and report the risk ratio and odds ratio with their confidence intervals using a GLM. For RRs you’ll need binomial with the log link; for ORs you’ll need binomial with logit links. You’ll need to need to exponentiate the estimates. Hint: Nesting functions can give you what you want: exp(coef(m)).
  
  
<div class="fold o"> 
```{r, echo=T, warning=F, message=F, results='asis', fig.width=11, fig.height=8}
# HW4.3.11
# m_crude_rr = glm(data=births, preterm_f ~ pnc5_f, family=binomial(link="log"))

m_crude_or = glm(data=births, preterm_f ~ pnc5_f, family=binomial(link="logit"))
exp(broom::confint_tidy(m_crude_or))
exp(confint(m_crude_or))

```
</div>


### Question 12, Likelihood Ratio Tests
1.  Compare at least two models using Likelihood Ratio Tests (LRTs). Try anova(model1, model2, test=”LRT”). Note that you’ll need to ensure they are nested. One way to do this is to first subset your data to include just (but all) of the variables you will use in your fullest model, then use na.omit() to drop records missing any of those variables, then pass that same restricted dataset to each of your glm() calls. You may note how mage linear mage performs without magesq, for instance.
