...
# set baselines
RD <- 0.5 # p(D = 1 | E = 1) -  p(D = 1 | E = 0)
pe <- 0.5 # prob of exposure
pd <- 0.3 # prob of dz

# p(E) = p(E | Z=1)p(Z=1) + p(E | Z=0)p(Z=0)
# p(D) = p(D | E=1)p(E=1 | Z=1)p(Z=1) + p(D | E=0)p(E=0 | Z=1)p(Z=1) +
# p(D | E=1)p(E=1 | Z=0)p(Z=0) + p(D | E=0)p(E=01 | Z=0)p(Z=0)


# confounding casually effects E (Z -> E)
# p(E|Z) = p(Z|E)*p(E) / p(Z)
df$exp[df$c == 1] <- rbinom(n=sum(df$c == 1), 1, (pze*pe)/pz)
df$exp[df$c == 0] <- rbinom(n=sum(df$c == 0), 1, ((1-pze)*pe)/pz)

# confounding casually effect D (arrow Z -> D)
# p(D | E, Z) = p(Z|E)*p(E) / p(Z)
df$dz[df$exp == 1 & df$c == 1] <- rbinom(sum(df$exp == 1 & df$c == 1), 1, prob = mean(df$exp[df$c == 1])*pz)
df$dz[df$exp == 0 & df$c == 1] <- rbinom(sum(df$exp == 0 & df$c == 1),1, prob = (1-mean(df$exp[df$c == 1]))*pz)
df$dz[df$exp == 1 & df$c == 0] <- rbinom(sum(df$exp == 1 & df$c == 0),1, prob = mean(df$exp[df$c == 1])*(1-pz))
df$dz[df$exp == 0 & df$c == 0] <- rbinom(sum(df$exp == 0 & df$c == 0),1, prob = (1-mean(df$exp[df$c == 1]))*(1-pz))
